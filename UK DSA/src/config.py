...
# ==============================================================================
# SECTION 10: ALIASES FOR RUN_ANALYSIS.PY (Expert-Corrected Format)
# ==============================================================================

# Convert fiscal year dict to calendar year format for econometric analysis
def _fy_to_cy(fy_dict):
    """Convert fiscal year keys to calendar year integers."""
    return {int(fy.split('-')[0]): val for fy, val in fy_dict.items()}

# Debt/GDP by calendar year (for Bohn test, econometric analysis)
HISTORICAL_DEBT_GDP = _fy_to_cy(HISTORICAL_DEBT_TO_GDP)

# Primary balance by calendar year (PSNB / GDP as %)
# Note: Primary balance = -PSNB/GDP (negative borrowing = positive balance)
HISTORICAL_PRIMARY_BALANCE = {
    int(fy.split('-')[0]): -HISTORICAL_PSNB[fy] / HISTORICAL_GDP[fy] * 100
    for fy in HISTORICAL_PSNB if fy in HISTORICAL_GDP
}

# GDP growth by calendar year (approximated from nominal GDP changes)
HISTORICAL_GDP_GROWTH = {}
prev_gdp = None
for fy in sorted(HISTORICAL_GDP.keys()):
    year = int(fy.split('-')[0])
    if prev_gdp is not None:
        growth = (HISTORICAL_GDP[fy] / prev_gdp - 1) * 100
        HISTORICAL_GDP_GROWTH[year] = growth
    prev_gdp = HISTORICAL_GDP[fy]

# 10Y gilt yields by calendar year
HISTORICAL_GILT_YIELDS = _fy_to_cy(HISTORICAL_GILT_YIELD_10Y)

# Budget projections by fiscal year string (for Monte Carlo)
BUDGET_PSND_GDP = OBR_DEBT_TO_GDP.copy()
BUDGET_PRIMARY_DEFICIT_GDP = {
    fy: -OBR_PSNB[fy] / OBR_GDP[fy] * 100 if fy in OBR_GDP else None
    for fy in OBR_PSNB
}

# Current debt/GDP
CURRENT_DEBT_GDP = HISTORICAL_DEBT_TO_GDP.get('2023-24', 96.0)
